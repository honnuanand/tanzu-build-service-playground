...
Create a new Resource Group in the specific region 
```bash
anandrao at Anands-MBP-3 in ~/pivotal/repos/k8s-playground/tanzu/tbs/azure (master●)
$ az group create --name tbs-on-aks --location westus2
{
  "id": "/subscriptions/1da11b37-d598-4c13-8523-690756fd00ab/resourceGroups/tbs-on-aks",
  "location": "westus2",
  "managedBy": null,
  "name": "tbs-on-aks",
  "properties": {
...
Update the local KUBECONFIG with the  credentials of the cluster
...
Now that we have  the aks cluster ready ,  lets intall [Tanzu Build Service](https://docs.pivotal.io/build-service/0-1-0/installing.html)

- Download duffle ,  pb cli from [pivnet](http://network.pivotal.io)
```bash
...
Confirm that you are on the correct kubernetis cluster
```bash
anandrao at Anands-MBP-3 in ~/pivotal/repos/k8s-playground/tanzu/tbs/azure (master●)
